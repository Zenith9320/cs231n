...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denom_k = torch.tensor(0.0, device=out.device)
        for i in range(2 * N):
            if i != k:
                denom_k += torch.exp(sim(z_k, out[i]) / tau)
        total_loss += -torch.log(num_k / denom_k)

        # l(k+N, k)
        num_kN = torch.exp(sim(z_k_N, z_k) / tau)
        denom_kN = torch.tensor(0.0, device=out.device)
        for i in range(2 * N):
            if i != k + N:
                denom_kN += torch.exp(sim(z_k_N, out[i]) / tau)
        total_loss += -torch.log(num_kN / denom_kN)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_sim = torch.cat([torch.diag(sim_matrix[:N, N:]), torch.diag(sim_matrix[N:, :N])], dim=0)  # [2N]
